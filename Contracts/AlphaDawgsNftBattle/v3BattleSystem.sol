...
contract AlphaDawgsBattleSystemBetaV7 is ReentrancyGuard, Ownable {
    ...
    struct Battle {
        ...
        bool[2] readyNow;
        bool completed;
        uint256 additionalPrize; // Tracks the additional prize for this battle
    }
    ...
    event BattleInitiated(uint256 indexed battleId, uint256 initiatedByTokenId);
    ...
    function enterBattle(uint256 tokenId) external payable {
        ...
        uint256 battleId = _findOrCreateBattle(tokenId);
        emit BattleInitiated(battleId, tokenId);
    }

    function _findOrCreateBattle(uint256 tokenId) private returns (uint256) {
    ...
    function startBattleManually(uint256 battleId) external {
        Battle storage battle = battles[battleId];
        require(block.timestamp >= battle.startTime + 1800, "Cannot start battle yet");
